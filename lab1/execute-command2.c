...
// for design problem shared memory
#include <sys/mman.h>
#include <semaphore.h>
#include <fcntl.h>
			...
			{		
				...
				if (childpid == 0) {
                  ...
                  free_command(c); // delete childes copy
                    ...
                    // increment shared total_subproc
                    sem_wait(&(shrptr->mutex));
                    shrptr->total_subproc++;
                    sem_post(&(shrptr->mutex));
...
/* Shared memory: increment in every fork
 * Decrement after every wait.
 */
...
execute_command(command_stream_t c_stream, bool time_travel, int N)
{
  ...
  error (1, 0, "command execution not yet implemented");*/
    ...
    // For shared memory, DESIGN PROBLEM
    int fd;
    //struct shared *shrptr;
    //sem_t mutex;
    ...
    if (time_travel) {
        ...
        if ((fd = open("blah.txt", O_RDWR | O_CREAT, 0666)) < 0) {
            fprintf(stderr, "Error: failed to open file for mmapping\n");
            exit(1);
        }
        write(fd, &shared, sizeof(struct shared));

        shrptr = (struct shared *) mmap(NULL, sizeof(struct shared), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        if ((int) shrptr == -1) {
        ...
        shrptr->total_subproc = 0;

        // Create, initialize semaphore
        if (sem_init(&(shrptr->mutex), 1, 1) < 0) {
            fprintf(stderr, "Error: could not initiliaze semaphore\n");
            exit(1);
        }
      ...
      while (commands_not_finished > 0) {
        ...
        for (i = 0; i < folder_count; i++) {
          ...
          if ( (wait_queue[i] == 0) && (c_stream->cmds[i]->status == 2)
                 && (shrptr->total_subproc + c_stream->cmds[i]->num_subproc <= N) ) {

              printf("Executing top level command type: %d, Total number of subprocesses: %d\n", c_stream->cmds[i]->type, shrptr->total_subproc);
              sem_post(&(shrptr->mutex));
        ...
        while ( (pid = wait(&status)) > 0) {
          ...
          //Decrement the shared total_subproc counter
          sem_wait(&(shrptr->mutex));
          shrptr->total_subproc--;
          sem_post(&(shrptr->mutex));
          ...
          for (j = 0; j < folder_count; j++) {
              ...
              if ( (wait_queue[j] == 0) && (c_stream->cmds[j]->status == 2)
                     && (shrptr->total_subproc + c_stream->cmds[j]->num_subproc <= N) ) {
                  printf("Executing top level command type: %d, Total number of subprocesses: %d\n", c_stream->cmds[j]->type, shrptr->total_subproc);
                  sem_post(&(shrptr->mutex));
            ...
            if (c_stream->cmds[i]->status == -1) {
              ...
              for (j = 0; j < folder_count; j++) {
                ...
                if (dep_graph[j][i]) {
                  ...
                  sem_wait(&(shrptr->mutex));
                  if ( (wait_queue[j] == 0) && (c_stream->cmds[j]->status == 2) && (shrptr->total_subproc + c_stream->cmds[j]->num_subproc <= N) ) {
                      printf("Executing top level command type: %d, Total number of subprocesses: %d\n", c_stream->cmds[i]->type, shrptr->total_subproc);
                      sem_post(&(shrptr->mutex));
